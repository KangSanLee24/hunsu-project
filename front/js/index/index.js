...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /** TAB 관련 JS **/
  $(document).ready(function () {
    function setupTabHover(tabSelector, contentSelector) {
      $(tabSelector).hover(function () {
        var tabDataId = $(this).attr('data-tab');
  
        // 이전 탭과 콘텐츠 클래스 제거
        $(tabSelector).removeClass('current');
        $(contentSelector).removeClass('current');
    ...
    setupTabHover('ul.hot-post-rank-tabs li', '.tab-contents-box');
    setupTabHover('ul.point-rank-tabs li', '.tab-ranking-box');
    setupTabHover('ul.hashtag-rank-tabs li', '.tab-hashtag-box');
  });

  /** 페이지 로딩이 되면 실행할 함수들 **/
  // 1. HOT LIVECHAT LIST
  fetchHotLiveChats(10);
  // 2. HOT POST RANKING LIST
  fetchHotPosts('CHAT');
